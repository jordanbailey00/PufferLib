 ...
 *
 * SINGLE FILE for all player configuration. Change skills and gear here.
 * Training selects the active loadout at runtime from the Puffer config.
 */
...
#define FC_DEFAULT_LOADOUT FC_LOADOUT_MASORI_TBOW
...
static const FcLoadout FC_LOADOUTS[FC_NUM_LOADOUTS] = {
    ...
    /* [FC_LOADOUT_MASORI_TBOW] End-game — Masori (f) + Twisted Bow */
    {
        .name         = "B: Masori (f) + TBow",
        .weapon_name  = "Twisted bow",
        .player_model_id = FC_PLAYER_MODEL_BASE + FC_LOADOUT_MASORI_TBOW,
...
static inline int fc_loadout_id_valid(int loadout_id) {
    return loadout_id >= 0 && loadout_id < FC_LOADOUT_COUNT;
}

static inline int fc_loadout_id_or_default(int loadout_id) {
    return fc_loadout_id_valid(loadout_id) ? loadout_id : FC_DEFAULT_LOADOUT;
}

static inline const FcLoadout* fc_loadout_get(int loadout_id) {
    return &FC_LOADOUTS[fc_loadout_id_or_default(loadout_id)];
}
